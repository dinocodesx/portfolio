...
import { ArrowLeft, ShieldIcon, RotateCcw } from "lucide-react";
import { Link } from "react-router-dom";
import { containerVariants, itemVariants } from "../../../../constants/animations";
import { Alert as AlertAtom, AlertType } from "../../../../components/ui/Alert";
...
export function Alert() {
  ...
  const alertData = {
    info: {
      ...
      content: "We've detected that your workspace could be optimized for faster response times. Enable the new caching layer in settings."
    },
    success: {
      title: "Secure Connection Established",
      content: "All data transmission is now encrypted using end-to-end industry standards. Your session is protected."
    },
    warning: {
      title: "Database Migration Required",
      content: "Your current database schema is outdated. Update to avoid potential data inconsistency during high-density operations."
    },
    error: {
      title: "Deployment Failed",
      content: "The latest build could not be deployed due to a configuration mismatch in the kernel environment."
    }
  };
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-12">
          ...
          <div className="space-y-12">
            ...
            >
              <p>
                A study on <span className="text-white">critical communication</span>. 
                Unlike toasts, these alerts are designed for inline persistence. Click 
                the triggers below to see how different system states are communicated.
              </p>
            </motion.div>
            ...
            >
              <div className="grid grid-cols-2 gap-4 w-full max-w-md mb-12">
                {(['info', 'success', 'warning', 'error'] as AlertType[]).map((type) => (
                  <button
                    key={type}
                ...
                <AnimatePresence mode="wait">
                  {activeAlert ? (
                    ...
                    >
                      ...
                      <p className="text-[13px] font-medium uppercase tracking-[0.2em]">System Standby</p>
                    </motion.div>
                  )}
                </AnimatePresence>
            ...
            >
              <p>
                The alerts use a <code className="bg-white/10 px-1.5 py-0.5 rounded text-[13px] text-white/80">AnimatePresence</code> wrapper 
                to handle state changes smoothly. This pattern is ideal for <span className="text-white">inline feedback</span> where the 
                component needs to occupy space while providing critical system updates.
              </p>
            </motion.div>
          </div>
        </div>
