...
export function MultiStepForm() {
  ...
  return (
    <UiShowcaseLayout
      ...
      description={
        <p>
          A <span className="text-white">multi-step authentication</span> flow
          designed to reduce cognitive load. By breaking complex data entry into
          logical chunks, we maintain high completion rates and a premium user
          experience.
        </p>
      }
    >
                    ...
                    {[1, 2, 3].map((s) => (
                      <div
                        ...
                        className={`h-1 rounded-full transition-all duration-500 ${
                          s <= step ? "w-4 bg-blue-500" : "w-1 bg-white/10"
                        }`}
                      />
                    ))}
                  </div>
      ...
      >
        <p>
          The form utilizes{" "}
          <span className="text-white">directional animations</span> to provide
          spatial context during navigation. Each step is validated before
          progression, and the final state provides immediate positive
          reinforcement through a success animation.
        </p>
      </motion.div>
    </UiShowcaseLayout>
  );
...
function Step1({
  ...
  formData: FormData;
  updateFields: (f: Partial<FormData>) => void;
}) {
  return (
    <>
    ...
    <div className="space-y-5 py-2">
      <div className="p-3.5 bg-white/5 border border-white/10 rounded-2xl space-y-2.5">
        ...
        <p className="text-white/40 text-[11px] leading-relaxed">
          By completing this registration, you agree to our Terms of Service and
          Privacy Policy. We'll use your data to provide a personalized
          experience.
        </p>
      </div>
      <label className="flex items-start gap-3 cursor-pointer group">
        ...
        <span className="text-[12px] text-white/40 group-hover:text-white/60 transition-colors leading-snug">
          I have read and agree to the terms and conditions of the service.
        </span>
      </label>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        <h2 className="text-white text-2xl font-medium">Account created</h2>
        <p className="text-white/40 max-w-xs text-[15px]">
          Welcome aboard, {fullName.split(" ")[0]}! Your account is now active
          and ready to use.
        </p>
      </div>
      <button
        onClick={onReset}
        className="h-12 px-8 bg-white/5 border border-white/10 rounded-xl text-white hover:bg-white/10 transition-colors text-[13px] font-medium"
      >
        Go to dashboard
      </button>
    </motion.div>
